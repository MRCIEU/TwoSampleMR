    ...
    if (compareNA(dat[[paste0("units.", what)]][1], "log odds")) {
      ...
      if (!paste0("prevalence.", what) %in% names(dat)) {
        ...
        warning(paste0(
          "Assuming ",
          what,
          " prevalence of 0.1. Alternatively, add prevalence.",
          what,
          " column and re-run."
        ))
      }
      ...
      if (sum(ind1) > 0) {
        ...
        dat[[paste0("effective_n.", what)]][ind1] <- effective_n(
          dat[[paste0("ncase.", what)]][ind1],
          dat[[paste0("ncontrol.", what)]][ind1]
        )
      } else {
      ...
      dat[[paste0("rsq.", what)]] <- 2 *
        dat[[paste0("beta.", what)]]^2 *
        dat[[paste0("eaf.", what)]] *
        (1 - dat[[paste0("eaf.", what)]])
...
get_r_from_pn_less_accurate <- function(p, n) {
  ...
  p[p == 1] <- 0.999
  p[p == 0] <- 1e-200
  qval <- stats::qchisq(p, 1, lower.tail = FALSE) /
    (stats::qchisq(p, n - 2, lower.tail = FALSE) / (n - 2))
  ...
  if (any(index)) {
    ...
    for (i in seq_along(index)) {
      if (p[index[i]] == 0) {
        ...
        R2[index[i]] <- suppressWarnings(
          stats::optim(
            0.001,
            optim.get_p_from_rn,
            sample_size = n[index[i]],
            pvalue = p[index[i]]
          )$par
        )
      }
...
contingency <- function(af, prop, odds_ratio, eps = 1e-15) {
  ...
  y <- vapply(
    z,
    function(a) zapsmall(matrix(c(a, prop - a, af - a, 1 + a - af - prop), 2, 2)),
    matrix(0.0, 2, 2)
  )

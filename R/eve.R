mr_mean_ivw <- function(d) {
  ...
  ivwoutliers <- dplyr::tibble(
    id.exposure = id.exposure,
    id.outcome = id.outcome,
    SNP = d$SNP,
    Qj = weights2^2 * (ratios - stats::coefficients(ivw2)[1])^2,
...
mr_mean_egger <- function(d) {
  ...
  eggeroutliers <- dplyr::tibble(
    SNP = d$SNP,
    Qj = weights2^2 *
      (ratios - stats::coefficients(egger2)[1, 1] / weights2 - stats::coefficients(egger2)[2, 1])^2,
    Qpval = stats::pchisq(Qj, 1, lower.tail = FALSE)
  )

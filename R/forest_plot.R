...
mr_forest_plot <-
    function(name, eff_Col = "b", exposure_Name, outcome_Name, forest_Title = '', outfile_Name = 'annot_FP.pdf', left_Col_Names, left_Col_Titles = NULL, right_Col_Names = c("p", "n.out.cases", "n.out.controls", "samplesize"), right_Col_Titles =
                 NULL, debug = FALSE,  log_ES = FALSE, decrease = TRUE,  returnRobj = FALSE, se_Col = "se") {
        ...
        spacer <-
            function(exposure, eff_col, outcome, Data_Fm, decrease = TRUE) {
                ...
                spacing_vec <-
                    vector(mode = "numeric", length = N_space)
                content_list <- list()
                attr_list <- list()
                row_list <- list()
                idx <- 1
                for (subgp in unique(Data_Fm[,exposure])) {
                    ...
                    content_list[((idx + 1):(idx + n_std))] <-
                        Data_Fm[sg_idxs[std_sort], outcome]
        ...
        ### 4. Generate a properly-spaced forest plot (without a summary effect) for all studies
        ggforest <-
            function(data_Fm, space_col, eff_col, lb_col = "lb", ub_col = "ub", title_text = '', log_ES = FALSE) {
        ...
        space1 <-
            spacer(
                exposure = exposure_Name, eff_col = eff_Col, outcome = outcome_Name, Data_Fm = data, decrease = decrease
            )

...
spacer <-
    function(sgname, eff_col, stdname, Data_Fm, decrease = TRUE) {
        # sgname = (character), column name of the column containing the type of study
        # eff_col = (character), column name of the column giving the effect size of interest
        # stdname = (character), column name giving study names/phenotype names
        ...
        N_sub <- length(unique(Data_Fm[,sgname]))
        ...
        for (subgp in unique(Data_Fm[,sgname])) {
            ...
            content_list[((idx + 1):(idx + n_std))] <-
                Data_Fm[sg_idxs[std_sort], stdname]
        ...
        return(
            data.frame(
                spacing_vec = (1 + length(spacing_vec) - spacing_vec), content_list = unlist(content_list), row_list = unlist(row_list), attr_list = unlist(attr_list)
            )
        )
    }
...
### 4. Generate a properly-spaced forest plot (without a summary effect) for all studies
ggforest <-
    function(data_Fm, space_col, eff_col, lb_col, ub_col, title_text = '', log_ES = FALSE) {
        ...
        raw_forest  <-
            ggplot(data = data_Fm, aes(
                y = space_col, yend = space_col, x = as.numeric(lb_col), xend = as.numeric(ub_col)
            )) + geom_segment() + geom_point(aes(
                y = space_col,  x = as.numeric(eff_col), size = 4
            )) + theme_bw() + theme(
                axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title = element_blank(), panel.grid = element_blank(), rect = element_blank(), title = element_text(size = 23), legend.position = 'none'
            ) + expand_limits(y = c(data_Fm[,space_col] - 1, data_Fm[,space_col] + 2)) + labs(title = title_text)
...
anot_col <-
    function(data_Fm, text_col, space_col, title_text = '') {
        ...
        text_widths <-
            c(-1, max(10,0.5 * max(sapply(
                as.character(data_Fm[,text_col]),nchar
            ))))

        # GGplot rendering of the annotation column
        lefttext  <-
            ggplot(data = data_Fm, aes(
                y = space_col, x = 0, label = text_col, fontface = attr_list
            )) + geom_text(hjust = 0) + theme_bw() + theme(
                axis.text.y = element_blank(), axis.ticks.y = element_blank(),axis.text.x = element_text(colour = "white"),axis.ticks.x = element_line(colour = "white"), axis.title = element_blank(), rect = element_blank(), panel.grid = element_blank(), title = element_text(size = 23)
            ) + expand_limits(x = text_widths,  y = c(data_Fm[,space_col] - 1, data_Fm[,space_col] + 2)) + labs(title = title_text, size = 40)
...
anot_side <-
    function(data_Fm, space_col, col_names, title_list = '') {
        ...
        relative_widths <-
            vector(mode = "numeric", length = length(col_names))
        ...
        for (i in 1:length(col_names)) {
            ...
            col <-
                anot_col(
                    data_Fm = data_Fm, text_col = col_names[i], space_col = space_col, title_text = title_list[[i]]
                )
            relative_widths[i] <-
                col$text_widths[2] - col$text_widths[1]
        ...
        output$relative_widths <-
            relative_widths / (sum(relative_widths)) * 0.33
...
group_Plots <- function(forst_Pt, left_Hs, right_Hs) {
    ...
    for (i in 1:length(right_Grobs)) {
        grob_Bag[paste('r',names(right_Grobs)[i], sep = '')] <-
            right_Grobs[i]
    }
    ...
    grp_FP <-
        gtable_matrix(
            name = "groupplot", grobs = matrix(grob_Bag, nrow = 1), widths = unit(width_vec, "npc"), heights = unit(1,"npc")
        )
...
forest_Plotter <-
    function(name, inRobj = FALSE, eff_Col, lb_Col, ub_Col, stdyGp_Name, study_Name, forest_Title = '', outfile_Name = 'annot_FP.pdf', left_Col_Names, left_Col_Titles = NULL, right_Col_Names, right_Col_Titles =
                 NULL, debug = FALSE, wmf = FALSE, log_ES = FALSE, decrease = TRUE, isxlsx = FALSE, returnRobj = FALSE) {
        # name = (character) name of the delimited file containing all of the results on the first sheet (needs to have headers), or of the r object
        ...
        # lb_Col = (character) name of the column in the delimited file containing lower bounds on the effect sizes
        # ub_Col = (character) name of the column in the delimited file containing upper bounds on the effect sizes
        # stdyGp_Name = (character) name of the column in the delimited file containing the *types* of studies
        # study_Name = (character) name of the column in the delimited file containing the names of each study
        ...
        # wmf = (logical) output a windows metafile y/n?
        ...
        # isxlsx = (logical) is the source data file in xslx format y/n?
        ...
        ## read in data
        if(inRobj == FALSE){
           data  <- read_external(filename = name, isxlsx = isxlsx)
        } else {
            data <- name
        }
        ...
        space1 <-
            spacer(
                sgname = stdyGp_Name, eff_col = eff_Col, stdname = study_Name, Data_Fm = data, decrease = decrease
            )
        ...
        fo1  <-
            ggforest(
                data_Fm = expand_data, space_col = 'spacing_vec', eff_col = eff_Col, lb_col = lb_Col ,ub_col = ub_Col, log_ES = log_ES, title_text = forest_Title
            )

        ## Construct left-hand-side annotations
        left <-
            anot_side(
                data_Fm = expand_data, space_col = 'spacing_vec', col_names = c('content_list',left_Col_Names), title_list = c('Content',left_Col_Titles)
            )

        ## Construct right-hand-side annotations
        right <-
            anot_side(
                data_Fm = expand_data, space_col = 'spacing_vec', col_names = right_Col_Names, title_list = right_Col_Titles
            )

        ## group all plots together
        group <-
            group_Plots(forst_Pt = fo1, left_Hs = left, right_Hs = right)
        ...
        if (returnRobj == FALSE) {
            if (wmf == FALSE) {
                ...
                win.metafile(outfile_Name,width = 23.4, height = 16.5)
            }

...
### 1. Read in a set of meta-analysis data that are in the first sheet of an xlsx file (that has a header)
read_xlsx_MA <- function(filename){
    # This is just a wrapper for the read.xlsx function
    MA_data <- read.xlsx(file = filename, sheetIndex = 1, stringsAsFactors = FALSE)
    return(MA_data)
}
...
spacer <- function(sgname, eff_col, stdname, Data_Fm, decrease = TRUE){
    ...
    for (subgp in unique(Data_Fm[,sgname])){
        ...
        content_list[((idx + 1):(idx + n_std))] <- Data_Fm[sg_idxs[std_sort], stdname]
...
### 4. Generate a properly-spaced forest plot (without a summary effect) for all studies
ggforest <- function(data_Fm, space_col, eff_col, lb_col, ub_col, title_text = '', log_ES = FALSE){
...
forest_Plotter <- function(filename, eff_Col, lb_Col, ub_Col, stdyGp_Name, study_Name, forest_Title = '', outfile_Name = 'annot_FP.pdf', left_Col_Names, left_Col_Titles = NULL, right_Col_Names, right_Col_Titles=NULL, debug = FALSE, wmf = FALSE, log_ES = FALSE, decrease = TRUE){
    # filename = (character) name of the xslx file containing all of the results on the first sheet (needs to have headers)
    # eff_Col = (character) name of the column in the xslx file that contains the effect sizes
    # lb_Col = (character) name of the column in the xslx file containing lower bounds on the effect sizes
    # ub_Col = (character) name of the column in the xslx file containing upper bounds on the effect sizes
    # stdyGp_Name = (character) name of the column in the xslx file containing the *types* of studies
    # study_Name = (character) name of the column in the xslx file containing the names of each study
    ...
    # left_Col_Names = (character vector) vector containing the names of the left-hand-side annotation columns in the xslx file
    # left_Col_Titles = (character vector) vector containing the titles for each left-hand-side annotation column
    # right_Col_Names = (character vector) vector containing the names of the right-hand-side annotation columns in the xslx file
    ...
    ## act on debug flag
    if (debug == FALSE) {options(warn=-1)}

    ## read in data
    data  <- read_xlsx_MA(filename)

    ## order and structure effect sizes and CIs for forest plot
    space1 <-spacer(sgname = stdyGp_Name, eff_col = eff_Col, stdname = study_Name, Data_Fm = data, decrease = decrease)

    expand_data  <- space_Out(data_Fm = data, space_Fm = space1)

    ## Make the forest plot
    fo1  <- ggforest(data_Fm = expand_data, space_col = 'spacing_vec', eff_col = eff_Col, lb_col = lb_Col ,ub_col = ub_Col, log_ES = log_ES, title_text = forest_Title)

    ## Construct left-hand-side annotations
    left <- anot_side(data_Fm = expand_data, space_col = 'spacing_vec', col_names = c('content_list',left_Col_Names), title_list = c('Content',left_Col_Titles) )

    ## Construct right-hand-side annotations
    right <-anot_side(data_Fm = expand_data, space_col = 'spacing_vec', col_names = right_Col_Names, right_Col_Titles )

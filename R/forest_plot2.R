...
forest_plot_ncase <- function(dat, section=NULL, bottom=TRUE)
{
	...
	ggplot2::labs(y=NULL, x=xlabname, colour="", fill=NULL, title="# cases") +
	outcome_labels
	return(p)
}


# forest_plot_newvalue <- function(dat, section=NULL, bottom=TRUE)
# {
# 	if(bottom)
# 	{
# 		text_colour <- ggplot2::element_text(colour="white")
...
# 		# strip.background = ggplot2::element_blank()
# 	) +
# 	ggplot2::labs(y=NULL, x=xlabname, colour="", fill=NULL, title="new value") +
# 	outcome_labels
# 	return(p)
# }
		...
		return(
			cowplot::plot_grid(
				gridExtra::arrangeGrob(
					grobs=l, 
					# change this to 3
					ncol=length(columns) + 2, 
					nrow=length(h), 
					heights=h,
					# add an extra width value
					widths=c(4, 1, rep(5, length(columns)))
				)
			)
		)
	}

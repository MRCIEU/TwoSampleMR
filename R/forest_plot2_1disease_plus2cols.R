...
forest_plot_tv <- function(dat, section=NULL, bottom=TRUE)
{
	if(bottom)
	{
		text_colour <- ggplot2::element_text(colour="white")
...
forest_plot <- function(mr_res, exponentiate=FALSE, single_snp_method="Wald ratio", multi_snp_method="Inverse variance weighted", group_single_categories=TRUE, by_category=TRUE, in_columns=FALSE, threshold=NULL, xlab="", xlim=NULL, trans="identity",ao_slc=T, priority="Cardiometabolic",Lo=Lo,Up=Up,name.length=1)
{
	requireNamespace("ggplot2", quietly=TRUE)
		...
		return(
			cowplot::plot_grid(
				gridExtra::arrangeGrob(
					...
					widths=c(name.length, 0.40,0.40, rep(5, length(columns)))
				)
			)
		)
	}

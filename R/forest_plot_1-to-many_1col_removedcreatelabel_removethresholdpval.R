...
#' @export
#' @return data frame.
format_mr_results <- function(mr_res, exponentiate=FALSE, ao_slc=T,by=NULL,TraitM="outcome",TraitM_label="outcome2")
{
	...
	dat <- data.frame(
		...
		outcome2= as.character(dat[,TraitM_label]),
...
#' @param Sort.action Choose how to sort results. 1 =sort results by effect size within groups (default). 2=sort results by traits. For exmaple, option 2 is preferable if a user wishes to plot multiple results for the same trait side by side (e.g. results for the same trait from observational and Mendelian randomization studies). 
...
Sort.1.to.many<-function(mr_res,Sort.action=1,Group=NULL,Priority=NULL){
...
forest_plot <- function(mr_res, exponentiate=FALSE, xlab="", xlim=NULL, trans="identity",ao_slc=T,Lo=Lo,Up=Up,ncols=1,var2="",var3="",var4="",width1=1,width2=1,width3=1,width4=1,by=NULL,TraitM="outcome",TraitM_label="outcome",Group="study")
{
	requireNamespace("ggplot2", quietly=TRUE)

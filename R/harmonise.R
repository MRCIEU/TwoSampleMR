...
harmonise_data <- function(exposure_dat, outcome_dat, action = 2) {
  ...
  if (length(action) == 1) {
    ...
    stop(
      "Action argument must be of length 1 (where the same action will be used for all outcomes), or number of unique id.outcome values (where each outcome will use a different action value)"
    )
  }
  ...
  for (i in seq_len(nrow(combs))) {
    ...
    attr(x, "log")[['variants_absent_from_reference']] <- sum(
      exposure_dat$id.exposure == x$id.exposure[1]
    ) -
      nrow(x)
    ...
    attr(x, "log")[["proxy_variants"]] <- ifelse(
      is.null(x$proxy.outcome),
      0,
      sum(x$proxy.outcome, na.rm = TRUE)
    )
    fix.tab[[i]] <- x
  }
...
check_required_columns <- function(dat, type = "exposure") {
  ...
  if (!all(index)) {
    stop(
      ...
      type,
      ": ",
      paste(required_columns[!index], collapse = ", ")
    )
  }
  return(NULL)
}

...
mr_report <- function(
  ...
  for (i in seq_len(nrow(combinations))) {
    ...
    tablist <- lapply(
      m[c("mr", "enrichment", "directionality_test", "mr_heterogeneity", "mr_pleiotropy_test")],
      function(x) {
        if (is.null(x)) {
          return(NULL)
        } else {
          subset(
            x,
            id.exposure == combinations$id.exposure[i] & id.outcome == combinations$id.outcome[i],
    ...
    plotlist <- lapply(p, function(x) {
      ...
      index <- which(
        d$id.exposure == combinations$id.exposure[i] & d$id.outcome == combinations$id.outcome[i]
      )
      if (length(index) < 1) {
    ...
    output_file[i] <- file.path(
      output_path,
      paste("TwoSampleMR", sanitise_string(title), output_type, sep = ".")
    )

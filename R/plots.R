...
mr_scatter_plot <- function(mr_results, dat, ci=FALSE)
{
	...
	mrres <- plyr::dlply(dat, c("id.exposure", "id.outcome"), function(d)
	{
		...
		mr_scatter_plot <- ggplot2::ggplot(data=d, ggplot2::aes(x=beta.exposure, y=beta.outcome)) +
			...
			ggplot2::labs(colour="MR Estimator", x=paste("SNP effect on", d$exposure[1]), y=paste("SNP effect on", d$outcome[1])) +
			ggplot2::theme(legend.position="top", legend.direction="vertical") +
			ggplot2::guides(colour=ggplot2::guide_legend(ncol=2))
                if (ci == TRUE & "MR Egger" %in% mrres$method) {
                        mr_scatter_plot <- mr_scatter_plot + 
                                ggplot2::geom_line(ggplot2::aes(y=lwr), linetype="dashed", colour="#1f78b4") + 
                                ggplot2::geom_line(ggplot2::aes(y=upr), linetype="dashed", colour="#1f78b4")
                }
                mr_scatter_plot
	})
        mrres

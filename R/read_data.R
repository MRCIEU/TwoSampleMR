...
format_data <- function(
  ...
  if (!all(mr_cols_required %in% names(dat))) {
    warning(
      "The following columns are not present and are required for MR analysis\n",
  ...
  if (samplesize_col %in% names(dat)) {
    ...
    if ("ncontrol.outcome" %in% names(dat) && "ncase.outcome" %in% names(dat)) {
      index <- is.na(dat$samplesize.outcome) &
...
combine_data <- function(x) {
  stopifnot(is.list(x))
  if ("exposure" %in% names(x[[1]])) {
    type <- "exposure"
  } else if ("outcome" %in% names(x[[1]])) {
    type <- "outcome"
  } else {

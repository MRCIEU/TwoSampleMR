...
mr_rucker_bootstrap <- function(dat, parameters=default_parameters())
{
	...
	rucker$bootstrap_results <- res
	rucker$bootstrap_estimates <- modsel
	rucker$bootstrap_q <- bootstrap
	rucker$q_plot <- p1
	rucker$e_plot <- p2
...
#' @param parameters=default_parameters() <what param does>
#' @param method="simple" <what param does>
#'
#' @importFrom magrittr "%>%"
#' @export
#' @return
rucker_function <- function(dat, parameters=default_parameters(), method="simple")
{
	...
	{
		fn <- mr_rucker
	} else if(method == "coodsdistance") {
		fn <- mr_rucker_cooksdistance
	} else {
	...
	selected <- lapply(out, function(x) {
		a <- x$selected
		a$choice <- x$res
		return(a)
		})
	allres <- lapply(out, function(x) x$rucker)
	selected <- plyr::rbind.fill(selected)
	allres <- plyr::rbind.fill(allres)
}

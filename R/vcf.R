...
#' @param rsid=TRUE Whether the snplist is rs ID (if FALSE then assume data frame of chrom and pos)
#' @param sel='%CHROM %POS %ID %REF %ALT %B %SE %PVAL %N %AF\n' What to extract from bcf
#'
#' @export
#' @return data frame
extract_from_bcf <- function(snplist, bcf, out, rsid=TRUE, sel='%CHROM %POS %ID %REF %ALT %B %SE %PVAL %N %AF\n')
{
	require(data.table)
...
#' @export
#' @return data frame
get_ld_proxies <- function(snplist, bcf, bfile, out, tag_kb=5000, tag_nsnp=5000, tag_r2=0.6)
{
	require(dplyr)
	require(data.table)
	...
	system(cmd)
	write.table(snplist, file=targetsname, row=FALSE, col=FALSE, qu=FALSE)
	...
	unlink(targetsname)
	unlink(paste0(targetsname, ".log", ".nosex"))
	unlink(outname)
	return(ld)
}
...
align_proxies <- function(ld, e, vcf_ref=NULL, tempfile=NULL)
{
	...
	ref <- extract_from_bcf(subset(r, select=c(CHROM, POS)), vcf_ref, tempfile, rsid=FALSE, '%CHROM %POS %ID %REF %ALT\n')
...
#' @export
#' @return
wrapper <- function(bcf, snplist, tempname, proxies="yes", bfile, vcf_ref, tag_kb=5000, tag_nsnp=5000, tag_r2=0.6)
{

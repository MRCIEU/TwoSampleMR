...
test_that("MR scatter plot for mr_ivw", {
  ...
  p <- expect_no_error(mr_scatter_plot(m, dat))
...
test_that("Scatter plot for default set of estimates", {
  # dat <- make_dat("ieu-a-2", "ieu-a-7")
  m2 <- mr(dat)
  p2 <- expect_no_error(mr_scatter_plot(m2, dat))
...
test_that("Scatter plot for mr_grip", {
  ...
  p3 <- expect_no_error(mr_scatter_plot(m3, dat))
...
test_that("A second scatter plot for mr_grip", {
  ...
  p4 <- expect_no_error(mr_scatter_plot(m4, dat2))
...
test_that("Forest plot", {
  p5 <- expect_no_error(mr_forest_plot(res_single))
...
test_that("Funnel plot", {
  p7 <- expect_no_error(mr_funnel_plot(res_single))
